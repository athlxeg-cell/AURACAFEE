...
function renderSections() {
  ...
  grid.innerHTML = visible.map((sec, i) => {
    ...
    const bg   = sec.image
      ? `<img class="sec-card-bg" src="${imgSrc(sec.image)}" alt="${name}" loading="lazy">`
...
function renderCategories(secId) {
  ...
  if (sec) {
    ...
    icon.innerHTML = sec.image
      ? `<img src="${imgSrc(sec.image)}" alt="${t(sec,'name')}" loading="lazy">`
      : emojis[secId] || '📋';
  ...
  grid.innerHTML = cats.map((cat, i) => {
    ...
    const img   = cat.image
      ? `<img class="cat-card-bg" src="${imgSrc(cat.image)}" alt="${name}" loading="lazy">`
...
function renderItems(catId) {
  ...
  if (cat) {
    ...
    catImg.innerHTML = cat.image
      ? `<img src="${imgSrc(cat.image)}" alt="${t(cat,'name')}" loading="lazy">`
      : '🍽️';
  }
...
function renderItemCards(items) {
  ...
  grid.innerHTML = list.map((item, i) => {
    ...
    const imgHtml = item.image
      ? `<img src="${imgSrc(item.image)}" alt="${name}" loading="lazy" style="${imgStyles.join(';')}">`
...
function openDetail(itemId) {
  ...
  $('detail-img-wrap').innerHTML = item.image
    ? `<img src="${imgSrc(item.image)}" alt="${name}" style="${detailStyles.join(';')}">`
